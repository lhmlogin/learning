...
class MainWindow:
    def __init__(self):
        self.gladefile="/workfile/maintain_scripts/check_regression_version/gui.xml"
        self.wTree = gtk.glade.XML(self.gladefile, "form")
        dic = {
        "on_form_destroy" : gtk.main_quit,
        "on_exit_clicked" : gtk.main_quit,
        "on_submit_clicked" : self.submit_clicked,
        "on_form_show":self.form_show,
        "on_combo_from_changed":self.combo_from_changed,
        "on_combo_mode_changed":self.combo_mode_changed,
        "on_button_select_clicked":self.button_select_clicked,
        "on_ent_filter_activate":self.ent_filter_activate,
        "on_button_right1_clicked":self.button_right1_clicked,
        "on_button_right2_clicked":self.button_right2_clicked,
        "on_button_delete_clicked":self.button_delete_clicked,
        "on_manual_add_clicked":self.button_manual_add_clicked
        }
        self.wTree.signal_autoconnect(dic)
        self.form = self.wTree.get_widget("form")
        self.form.set_position(gtk.WIN_POS_CENTER)
        self.form.show()
    def form_show(self, widget):
        ...
        treeview_versionlist = self.wTree.get_widget("treeview_version_list")
        ...
        treeview_compare_version = self.wTree.get_widget("treeview_compare_version")
    ...
    def combo_from_changed(self, widget):
        #if the value is file, show the hbox_file, else hide the hbox_file
        combo_from = self.wTree.get_widget("combo_from")
        combo_from_name = combo_from.get_active_text()
        if combo_from_name == "file":
            #self.wTree.get_widget("hbox_file").show()
            self.wTree.get_widget("hbox_file").show()
        else:
            #self.wTree.get_widget("hbox_file").hide()
            self.wTree.get_widget("hbox_file").hide()

    def combo_mode_changed(self, widget):
        #change the label name just below it
        combo_mode = self.wTree.get_widget("combo_mode")
        combo_mode_index = combo_mode.get_active()
        if combo_mode_index == 0:
            self.wTree.get_widget("label_num").show()
            self.wTree.get_widget("label_x").hide()
            self.wTree.get_widget("label_mark").hide()
            self.wTree.get_widget("label_on").show()
            self.wTree.get_widget("label_y").hide()
            self.wTree.get_widget("ent_y").show()
        elif combo_mode_index == 1:
            self.wTree.get_widget("label_num").hide()
            self.wTree.get_widget("label_x").show()
            self.wTree.get_widget("label_mark").hide()
            self.wTree.get_widget("label_on").hide()
            self.wTree.get_widget("label_y").show()
            self.wTree.get_widget("ent_y").show()
        elif combo_mode_index == 2:
            self.wTree.get_widget("label_num").hide()
            self.wTree.get_widget("label_x").hide()
            self.wTree.get_widget("label_mark").show()
            self.wTree.get_widget("label_on").hide()
            self.wTree.get_widget("label_y").hide()
            self.wTree.get_widget("ent_y").hide()
        else:
            self.wTree.get_widget("label_num").show()
            self.wTree.get_widget("label_x").hide()
            self.wTree.get_widget("label_mark").hide()
    ...
    def button_select_clicked(self, widget):
        ...
        if response == 0:
            ...
            filename = ''
            fcd.destroy()
        self.wTree.get_widget("ent_file").set_text(filename)

    def ent_filter_activate(self, widget):
        #filter the version list
        self.version_filter=self.store_vl.filter_new()
        self.filter_text=self.wTree.get_widget("ent_filter").get_text()
    ...
    def button_right1_clicked(self, widget):
        ...
        if len(pathlist) == 1:
            ...
            self.wTree.get_widget("ent_current_version").set_text(value)
        else:
            self.raise_alert("Only one row can be selected")
            #print "only one row can be selected"
        
    def button_right2_clicked(self, widget):
    ...
    def button_manual_add_clicked(self,widget):
        manual_version_text=self.wTree.get_widget("manual_version").get_text()
    ...
    def get_information(self):
        self.ent_job_text=self.wTree.get_widget("ent_job").get_text()
        self.ent_step_text=self.wTree.get_widget("ent_step").get_text()
        self.ent_chk_text=self.wTree.get_widget("ent_chk").get_text()
        self.ent_layer_text=self.wTree.get_widget("ent_layer").get_text()
        self.combo_from_text=self.wTree.get_widget("combo_from").get_active_text()
        self.combo_mode_text=self.wTree.get_widget("combo_mode").get_active_text()
        self.ent_x_text=self.wTree.get_widget("ent_x").get_text()
        self.ent_y_text=self.wTree.get_widget("ent_y").get_text()
        self.ent_file_text=self.wTree.get_widget("ent_file").get_text()
        self.ent_current_version_text=self.wTree.get_widget("ent_current_version").get_text()
